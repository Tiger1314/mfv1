...
//自定义校验规则
//验证手机号
var mobile = function(value, ref){
    return value.length == 11 && /^((13|14|15|17|18)[0-9]{1}\d{8})$/.test(value)
}
//验证正浮点数
var fds = function (value, ref) {
...
/* eslint-disable max-len */
var messages = {
  alpha_dash: function (field) { return ("能够包含字母数字字符，包括破折号、下划线."); },
  alpha_num: function (field) { return ("只能包含字母数字字符."); },
  alpha_spaces: function (field) { return ("只能包含字母字符，包括空格."); },
  alpha: function (field) { return ("只能包含字母字符."); },
  ...
  confirmed: function (field) { return ("不匹配."); },
  credit_card: function (field) { return ("无效."); },
  decimal: function (field, ref) {
    if ( ref === void 0 ) ref = ['*'];
    var decimals = ref[0];

    return ("必须是数字的而且能够包含 " + (decimals === '*' ? '' : decimals) + " 位小数.");
},
  digits: function (field, ref) {
    var length = ref[0];

    return ("必须是数字，且精确到 " + length + " 位数.");
},
  dimensions: function (field, ref) {
    var width = ref[0];
    var height = ref[1];

    return ("必须是 " + width + " 像素到 " + height + " 像素.");
},
  email: function (field) { return ("请填写正确的邮件格式."); },
  ext: function (field) { return ("必须是有效的文件."); },
  image: function (field) { return ("必须是图片."); },
  in: function (field) { return ("必须是一个有效值."); },
  ip: function (field) { return ("必须是一个有效的地址."); },
  max: function (field, ref) {
    var length = ref[0];

    return ("不能大于 " + length + " 字符.");
},
  ...
  mimes: function (field) { return ("必须是有效的文件类型."); },
  min: function (field, ref) {
    var length = ref[0];

    return ("必须大于" + length + " 字符.");
},
  ...
  not_in: function (field) { return ("必须是一个有效值."); },
  numeric: function (field) { return ("只能包含数字."); },
  regex: function (field) { return ("格式无效."); },
  required: function (field) { return ("必填."); },
  size: function (field, ref) {
    var size = ref[0];

    return ("不能超过" + size + " KB.");
},
  url: function (field) { return ("URL无效."); },
  mobile : function (field){return ("手机号无效.")},
  fds : function (field){return ("请输入正确的数字.")}
};
